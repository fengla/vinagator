...
<body>
                        ...
                        <div class="ibox float-e-margins">
                            ...
                            <div class="ibox-content" style="padding:0">
                                <table class="table table-striped" style="word-break:break-all; word-wrap:break-word; font-size: 15px">
                                    ...
                                    <tbody>
                                    <c:forEach items="${resp.content}" var="banner">
                                        <tr>
    ...
    <script>
        ...
        pageDiv = "<div class=\"text-center\">\n" +
            "                                    <div class=\"btn-group\">\n" +
            "                                        <a class=\"btn btn-white\" href=\"/editBanner?curPage=0\">第一页</a>";

        if(${resp.number} > 0){
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number-1}\">上一页</a>";
        }
        if(${resp.number} > 1){
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number-2}\">${resp.number-1}</a>\n" +
                "                                        <a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number-1}\">${resp.number}</a>";
        }else if(${resp.number} > 0){
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number-1}\">${resp.number}</a>";
        }

        pageDiv += "<a class=\"btn btn-white active\" href=\"/editBanner?curPage=${resp.number}\">${resp.number+1}</a>";

        if(${resp.number+1} < ${resp.totalPages-1}){
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number+1}\">${resp.number+2}</a>\n" +
                "                                        <a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number+2}\">${resp.number+3}</a>";
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number+1}\">下一页</a>";
        }else if(${resp.number+1} < ${resp.totalPages}){//当前页是倒数第二页
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number+1}\">${resp.number+2}</a>";
            pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.number+1}\">下一页</a>";
        }

        pageDiv += "<a class=\"btn btn-white\" href=\"/editBanner?curPage=${resp.totalPages-1}\">最后一页</a>";
