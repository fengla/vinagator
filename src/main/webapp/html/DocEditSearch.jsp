...
<body>
                    ...
                    <div class="search-form">
                        <form action="/findDocsByKeyword" method="get">
                            <select id="type" name="searchType" class="col-lg-1" style="height:46px;background-color:#1ab394;border-radius:0px;color:white;font-size:17px">
                                <option id="docId" value="docId">docId</option>
                                <option id="title" value="title">标题</option>
                            </select>
                            <div class="input-group">
                                <input type="text" placeholder="${keyword}" name="keyword" class="form-control input-lg">
    ...
    <script>
        ...
        pageDiv = "<div class=\"text-center\">\n" +
            "                                    <div class=\"btn-group\">\n" +
            "                                        <a class=\"btn btn-white\" href=\"/findDocsByKeyword?currentPage=1\">第一页</a>";

        if(${pageModel.currentPage} > 1){
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage-1}\">上一页</a>";
        }
        if(${pageModel.currentPage} > 2){
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage-2}\">${pageModel.currentPage-2}</a>\n" +
                "                                        <a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage-1}\">${pageModel.currentPage-1}</a>";
        }else if(${pageModel.currentPage} > 1){
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage-1}\">${pageModel.currentPage-1}</a>";
        }

        pageDiv += "<a class=\"btn btn-white active\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage}\">${pageModel.currentPage}</a>";

        if(${pageModel.currentPage} < ${pageModel.totalPage-1}){
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage+1}\">${pageModel.currentPage+1}</a>\n" +
                "                                        <a class=\"btn btn-white\" href=\"/findDocsByKeyword?currentPage=${pageModel.currentPage+2}\">${pageModel.currentPage+2}</a>";
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage+1}\">下一页</a>";
        }else if(${pageModel.currentPage} < ${pageModel.totalPage}){//当前页是倒数第二页
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage+1}\">${pageModel.currentPage+1}</a>";
            pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.currentPage+1}\">下一页</a>";
        }

        pageDiv += "<a class=\"btn btn-white\" href=\"/findDocsByKeyword?keyword=${keyword}&&searchType=${searchType}&&currentPage=${pageModel.totalPage}\">最后一页</a>";

        document.getElementById("pageDiv").innerHTML = pageDiv;

        if(${searchType}==title){
            document.getElementById("title").setAttribute("selected","selected");
        }else if(${searchType}==docId){
            document.getElementById("docId").setAttribute("selected","selected");
        }
    </script>
